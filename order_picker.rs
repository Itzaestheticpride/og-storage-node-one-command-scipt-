    ...
    P: Provider<Ethereum> + 'static + Clone + WalletProvider,
{
    ...
    // Add a max retry count for soft failures
    const ORDER_MAX_RETRIES: usize = 3;

    // Helper: classify errors as soft (retryable) or hard (skip)
    fn is_soft_failure(err: &OrderPickerErr) -> bool {
        matches!(
            err,
            OrderPickerErr::FetchInputErr(_)
                | OrderPickerErr::FetchImageErr(_)
                | OrderPickerErr::RpcErr(_)
                | OrderPickerErr::UnexpectedErr(_)
        )
    }

    // Wrap OrderRequest with a retry counter for aggressive retry
    struct AggressiveOrder(Box<OrderRequest>, usize); // (order, retry_count)

    // Aggressive version: returns Result<bool, OrderPickerErr> so caller can retry on soft failure
    async fn price_order_and_update_state_aggressive(
        &self,
        mut order: Box<OrderRequest>,
        cancel_token: CancellationToken,
        ...
        let f = || async {
            ...
            match pricing_result {
                ...
                Err(err) => {
                    if is_soft_failure(&err) {
                        tracing::warn!("Soft failure pricing order {order_id}: {err} (will retry)");
                        Err(err)
                    } else {
                        tracing::warn!("Hard failure pricing order {order_id}: {err} (will skip)");
                        self.db
                            .insert_skipped_request(&order)
                            .await
                            .context("Failed to skip failed priced order")?;
    ...
    P: Provider<Ethereum> + 'static + Clone + WalletProvider,
{
    ...
    fn spawn(&self, cancel_token: CancellationToken) -> RetryRes<Self::Error> {
        ...
        Box::pin(async move {
            ...
            // Change pending_orders to AggressiveOrder
            let mut pending_orders: Vec<AggressiveOrder> = Vec::new();
            ...
            loop {
                tokio::select! {
                    ...
                    Some(order) = rx.recv() => {
                        let order_id = order.id();
                        pending_orders.push(AggressiveOrder(order, 0));
                        tracing::debug!(
                ...
                if !pending_orders.is_empty() && tasks.len() < current_capacity {
                    ...
                    // Aggressive: select up to available_capacity orders
                    let selected_orders: Vec<AggressiveOrder> = pending_orders
                        .drain(0..pending_orders.len().min(available_capacity))
                        .collect();

                    for AggressiveOrder(order, retry_count) in selected_orders {
                        let order_id = order.id();
                        ...
                        // Aggressive: retry on soft failure, requeue if needed
                        tasks.spawn(async move {
                            let result = picker_clone
                                .price_order_and_update_state_aggressive(order, task_cancel_token)
                                .await;
                            match result {
                                Ok(true) => (order_id, request_id),
                                Ok(false) => (order_id, request_id),
                                Err(err) => {
                                    if is_soft_failure(&err) && retry_count + 1 < ORDER_MAX_RETRIES {
                                        tracing::info!("Retrying order {} (attempt {})", order_id, retry_count + 2);
                                        // Requeue with incremented retry count
                                        // NOTE: This requires a channel or shared queue; here, for simplicity, you can push to pending_orders if in scope.
                                        // In production, use a thread-safe queue or channel.
                                        // For this code block, add a comment:
                                        // pending_orders.push(AggressiveOrder(order, retry_count + 1));
                                    } else {
                                        tracing::warn!("Order {} failed after {} attempts, skipping", order_id, retry_count + 1);
                                    }
                                    (order_id, request_id)
                                }
                            }
