    ...
    P: Provider<Ethereum> + 'static + Clone,
{
    ...
    /// Helper: robustly send an order to the queue, retrying if the channel is full or temporarily unavailable.
    async fn send_order_with_retry(
        ...
        let mut attempts = 0;
        loop {
            match new_order_tx.try_send(order.clone()) {
                Ok(_) => {
                    tracing::trace!("Order {} sent to queue", order_id);
                    return Ok(());
                }
                Err(mpsc::error::TrySendError::Full(_)) => {
                    if attempts < max_retries {
                        ...
                        return Err(MarketMonitorErr::ReceiverDropped);
                    }
                }
                Err(mpsc::error::TrySendError::Closed(_)) => {
                    tracing::error!("Order queue closed, cannot send order {}", order_id);
                    return Err(MarketMonitorErr::ReceiverDropped);
                }
            }
        }
    ...
    async fn find_open_orders(
        ...
        for log in decoded_logs {
            ...
            // Use robust send with retry
            if let Err(e) = Self::send_order_with_retry(Box::new(new_order), new_order_tx, &order_id, 5).await {
                tracing::error!("Failed to send open order {} to queue: {:?}", order_id, e);
            } else {
                order_count += 1;
            }
        }
                        ...
                        Some(Ok((event, log))) => {
                            ...
                            if event.prover != prover_addr {
                                ...
                                if let Some(order) = order {
                                    ...
                                    if let Err(e) = Self::send_order_with_retry(Box::new(order), &new_order_tx, &order_id, 5).await {
                                        tracing::error!("Failed to send order locked by another prover, {:x}: {:?}", event.requestId, e);
                                    }
                                } else {
