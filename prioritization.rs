    ...
    T: AsRef<OrderRequest>,
{
    ...
    // After sorting, move orders that are likely to fail lock (e.g., close to expiry) to the end
    orders.sort_by_key(|order| {
        ...
        // Orders expiring soon are deprioritized
...
/// Maximum number of retries for soft lock failures
const MAX_SOFT_LOCK_RETRIES: usize = 3;

/// Attempts to lock an order, retrying on soft lock failures.
/// Returns true if lock succeeded, false otherwise.
fn try_lock_order_with_retries(order: &OrderRequest, max_retries: usize) -> bool {
    let mut attempts = 0;
    while attempts < max_retries {
        // Replace this with actual lock logic
        let lock_result = try_lock_order(order);
        if lock_result.is_ok() {
            return true;
        } else if is_soft_lock_failure(&lock_result) {
            tracing::warn!("Soft lock failed for order {}, retrying ({}/{})", order.id(), attempts+1, max_retries);
            attempts += 1;
            std::thread::sleep(std::time::Duration::from_millis(100 * (attempts as u64)));
            continue;
        } else {
            tracing::error!("Hard lock failure for order {}: {:?}", order.id(), lock_result);
            break;
        }
    }
    false
...
// Dummy implementations for illustration; replace with actual logic.
fn try_lock_order(_order: &OrderRequest) -> Result<(), &'static str> {
    // ...simulate lock attempt...
    Ok(())
}
fn is_soft_lock_failure(_res: &Result<(), &'static str>) -> bool {
    // ...simulate detection...
    false
}
...
impl<P> OrderPicker<P> {
    ...
    pub(crate) fn select_pricing_orders(
        ...
        // Try to lock up to 'capacity' orders, retrying on soft lock failures
        let mut selected = Vec::new();
        let mut i = 0;
        while i < orders.len() && selected.len() < capacity {
            let order = &orders[i];
            if try_lock_order_with_retries(order, MAX_SOFT_LOCK_RETRIES) {
                selected.push(orders.remove(i));
            } else {
                i += 1;
            }
        }
